...
def guardar_registro(consultas, controles, mensajes, llamadas, canto, consultantes_texto):
    ...
    registro = {
        ...
        "canto": canto,
        "semana": semana,
        "consultantes": consultantes_texto,
...
def mostrar_registros_oficina():
    ...
    if registros:
        ...
        # Asegurar que consultantes se muestren con saltos de línea
        if "consultantes" in df.columns:
            df["consultantes"] = df["consultantes"].fillna("").astype(str)
    ...
    if not st.session_state.logged_in:
        ...
        if not st.session_state.is_admin:
            with st.form("form_registro"):
                ...
                consultantes_texto = st.text_area(
                    "👥 Consultantes (nombre y teléfono, uno por línea)", height=150
                )
