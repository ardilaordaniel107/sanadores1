...
# -------------------------------
# Expandir consultantes en columnas
# -------------------------------
def expandir_consultantes(df):
    if "consultantes" in df.columns:
        max_len = df["consultantes"].apply(lambda x: len(x) if isinstance(x, list) else 0).max()
        for i in range(max_len):
            df[f"Consultante{i+1}_Nombre"] = df["consultantes"].apply(
                lambda x: x[i]["nombre"] if isinstance(x, list) and i < len(x) else ""
            )
            df[f"Consultante{i+1}_Teléfono"] = df["consultantes"].apply(
                lambda x: x[i]["telefono"] if isinstance(x, list) and i < len(x) else ""
            )
    return df.drop(columns=["consultantes"])
...
def mostrar_registros_oficina():
    ...
    if registros:
        ...
        with c3: metric_card("✉️ Total Mensajes", total_mensajes)
    ...
    if not st.session_state.logged_in:
        ...
        if not st.session_state.is_admin:
            with st.form("formulario"):
                ...
                mensajes = st.number_input("✉️ Número de mensajes", min_value=0, step=1)
                ...
                for i in range(consultas):
                    nombre = st.text_input(f"Consultante {i+1} - Nombre", key=f"nombre_{i}")
                    telefono = st.text_input(f"Consultante {i+1} - Teléfono", key=f"telefono_{i}")
