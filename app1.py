...
def guardar_registro(consultas, controles, mensajes, llamadas, canto, consultantes_texto):
    ...
    # Guardar consultantes en tabla consultantes como texto
    if consultantes_texto.strip() != "":
        supabase.table("consultantes").insert({
            "registro_id": registro_id,
            "detalle": consultantes_texto
        }).execute()
...
def mostrar_registros_oficina():
    ...
    if registros:
        ...
        consultantes = supabase.table("consultantes").select("*").execute().data
...
def obtener_registros():
    try:
        ...
        consultantes = consultantes_resp.data

        consultantes_map = {}
        for c in consultantes:
            rid = c["registro_id"]
            if rid not in consultantes_map:
                consultantes_map[rid] = []
            consultantes_map[rid].append(c["detalle"])
        ...
        for reg in registros:
            ...
            st.write(f"⏰ Registrado en: {reg['fecha_registro']}")
