...
def guardar_registro(consultas, controles, mensajes, llamadas, canto, consultantes):
    ...
    # Guardar consultantes en tabla consultantes
    for idx, c in enumerate(consultantes, start=1):
        if c["nombre"].strip() != "" or c["telefono"].strip() != "":
    ...
    if not st.session_state.logged_in:
        ...
        if not st.session_state.is_admin:
            with st.form("form_registro"):
                ...
                num_consultantes = st.number_input("👥 Número de consultantes", min_value=0, step=1)
