...
def guardar_registro(consultas, controles, mensajes, llamadas, canto, consultantes):
    today = date.today()
    fecha = today.isoformat()  # Guardamos fecha exacta YYYY-MM-DD
    ...
    # Guardar consultantes como texto en el mismo registro
    if consultantes.strip() != "":
        supabase.table("consultantes").insert({
            "registro_id": registro_id,
            "detalle": consultantes
        }).execute()
    ...
    if not st.session_state.logged_in:
        ...
        if not st.session_state.is_admin:
            with st.form("form_registro"):
                ...
                consultantes = st.text_area("👥 Consultantes (nombres y teléfonos separados por línea)")
